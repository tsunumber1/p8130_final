...
logistics2 <- glm(Status ~ Age + Race + T.Stage + N.Stage + Grade + 
...
```{r logit_diagnostics}
test_model <- glm(Status ~ ., family = binomial, data = data_cleaned)
...
plot(test_model, which = 4, id.n = 5)
summary(test_model)
...
residuals <- residuals(logistics2, type = "pearson")
plot(residuals, main = "Residual Plot", xlab = "Index", ylab = "Residuals")
abline(h = 0, col = "red")

# existence of of strongly influential outliers
std_residuals <- rstandard(logistics2)
leverage <- hatvalues(logistics2)
cooks_distance <- cooks.distance(logistics2)
...
plot(logistics, which = 4, id.n = 3)
model.data <- augment(logistics) %>% 
  mutate(index = 1:n()) 
...
# try using lasso for model selection
```{r}
...
logistic1 <- glm(Status ~ Age + Race + Marital.Status + T.Stage + N.Stage + Grade + Tumor.Size + Estrogen.Status + Progesterone.Status + Regional.Node.Examined + Survival.Months, data = data_cleaned, family = binomial)
summary(logistic1)

#diagosis
std_residuals <- rstandard(logistic1)
leverage <- hatvalues(logistic1)
cooks_distance <- cooks.distance(logistic1)
plot_data <- data.frame(Leverage = leverage, Residuals = std_residuals)
