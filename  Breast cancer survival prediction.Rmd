...
logistics <- glm(Status ~ Age + Race + T.Stage + N.Stage + Grade + Estrogen.Status + 
    Progesterone.Status + Regional.Node.Examined + Survival.Months, family = binomial, data = data_cleaned)
summary(logistics)
```
...
residuals <- residuals(logistic, type = "pearson")
plot(residuals, main = "Residual Plot", xlab = "Index", ylab = "Residuals")
abline(h = 0, col = "red")

# existence of of strongly influential outliers
std_residuals <- rstandard(logistics)
leverage <- hatvalues(logistics)
cooks_distance <- cooks.distance(logistics)
